// Demo in-memory store for Intellectro social shell.
// Active only when Supabase public env is not configured (demo mode).
// When Supabase is configured, the shell uses the real data path.
// No secrets, no persistence across reloads — this is demo affordance.
...
function write(store) {
  ...
  try {
    ...
    // ignore quota / private-mode failures; demo continues in memory only
  }
}
...
export const demoStore = Object.freeze({
  ...
  addComment(postId, comment) {
    const store = raw() ?? { comments: { ...initial.comments }, claimResponses: [], reactions: {}, sources: [], approvals: [] };
  ...
  toggleReaction(postId, userId, reaction) {
    const store = raw() ?? { comments: { ...initial.comments }, claimResponses: [], reactions: {}, sources: [], approvals: [] };
    const key = `${postId}:${userId}:${reaction}`;
    if (store.reactions[key]) {
      delete store.reactions[key];
    } else {
      store.reactions[key] = true;
    }
    write(store);
  ...
  getReactionsForPost(postId) {
    ...
    for (const key of Object.keys(store.reactions)) {
      const [, , reaction] = key.split(':');
      if (reaction in out) out[reaction] += 1;
    }
  ...
  recordApproval(actionId, approverId, decision, note = '') {
    const store = raw() ?? { comments: { ...initial.comments }, claimResponses: [], reactions: {}, sources: [], approvals: [] };
    store.approvals.push(Object.freeze({ actionId, approverId, decision, note, createdAt: new Date().toISOString() }));
    write(store);
  },

  getApprovalsForAction(actionId) {
    const store = raw() ?? initial;
    return Object.freeze(store.approvals.filter((a) => a.actionId === actionId));
  },
});

export function useDemoStore() {
  // Client hook — returns a fresh snapshot each render.
  // Writes go through `demoStore` static methods which mutate localStorage.
